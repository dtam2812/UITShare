...
function verify2(options) {
  ...
  return secp256r12.verify(signature, payload instanceof Uint8Array ? payload : fromHex(payload), toHex3(publicKey).substring(2), { lowS: true, ...hash2 ? { prehash: true } : {} });
}
...
function getSignPayload2(envelope, options = {}) {
  ...
  if (options.from)
    return keccak256(concat("0x04", sigHash, resolve(options.from)));
  return sigHash;
}
