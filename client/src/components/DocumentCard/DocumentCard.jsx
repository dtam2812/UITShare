...
const DocumentCard = ({
  ...
  return (
    <Link
      to={`/documentDetail/${documentId}`}
      aria-label={`Xem tài liệu: ${title}`}
      className="block"
    >
      <div className="flex cursor-pointer flex-col overflow-hidden rounded-xl border border-white/10 bg-linear-to-br from-[#12121f] to-[#1a1a2e] shadow-md transition-all duration-300 hover:scale-[1.02] hover:shadow-xl">

        {/* Ảnh — thấp hơn trên mobile */}
        <div className="relative h-24 shrink-0 sm:h-32">
          ...
          />
          <div className="absolute top-2 right-2 rounded-full border border-white/20 bg-black/60 px-2 py-0.5 text-[10px] font-medium text-white shadow-sm">
            {categoryInfo.label}
          </div>
        </div>

        {/* Info */}
        <div className="flex flex-1 flex-col gap-1.5 border-b border-white/10 px-3 py-2.5 sm:gap-2 sm:px-4 sm:py-3">
          ...
          {/* Title */}
          <h2 className="line-clamp-2 text-xs leading-snug font-bold text-white sm:text-sm">
            {title?.split(".")[0]}
          </h2>
